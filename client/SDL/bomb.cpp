...
Bomb::Bomb( PlayerView* player,Camera& camera_reseiver, ManageTexture& manejador,GameConfig& config):
...
void Bomb::draw(SDL_Renderer& renderer){
   // std::cout << "activada"<< static_cast<int>(activada)<< std::endl;
    if(activada){
        ...
        origin_rect = {(item_sprite.x * width_img / 5), (item_sprite.y * height_img / 5), (width_img / 5), (height_img / 5)};
        SDL_RenderCopyEx(&renderer, texture_explosion, &origin_rect, &destination_rect, angle, nullptr, SDL_FLIP_NONE);
    } else {
        SDL_RenderCopyEx(&renderer, texture_bomb, nullptr, &destination_rect, angle, nullptr, SDL_FLIP_NONE);
    }
}

bool Bomb::explosion_finished(){
    return item_sprite.y == 5;
}
Bomb::~Bomb(){
