...
Bullet::Bullet(const Coordenada& start, const Coordenada& end,const  int& width,const WeaponCode& code ):
        current(start), end(end), width(width), code(code), start_time(SDL_GetTicks()) {}


void Bullet::calculate_values(SDL_Rect& destination_rect,double& angulo,SDL_Point& center, const SDL_Point& p_init, const SDL_Point& p_end){
    ...
    destination_rect.y = static_cast<int>(p_init.y - longitud - camera.y); // verticalmente centrado
...
SDL_Texture* Bullet::crear_cono(SDL_Renderer* renderer,SDL_Point p_inicial, SDL_Point p_final, double apertura_grados = 60, int paso = 1) {
    ...
    SDL_Texture* textura = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, ancho, alto);
