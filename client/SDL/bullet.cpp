...
Bullet::Bullet(const Coordenada& start, const Coordenada& end, SDL_Texture* texture):
        current(start), end(end), texture(texture) {}


void Bullet::draw(SDL_Renderer& renderer) {

    SDL_Rect bulletRect = {static_cast<int>(current.x * 32), static_cast<int>(current.y * 32), 15,
                           15};
    std::cout << "bulletRect -> x: " << bulletRect.x << ", y: " << bulletRect.y
              << ", w: " << bulletRect.w << ", h: " << bulletRect.h << std::endl;
...
bool Bullet::finalizado() { return current.x == end.x && current.y == end.y; }
