...
Shopping::Shopping(Camera& camera_receiver, ManageTexture& manejador, GameConfig& config)
    ...
    std::vector<WeaponData> weaponVector = {
        { Weapon::AWP, "AWP", 2100 ,"- Mayor  alcance  Daño  maximo ,- No  afectado  por  distancia ,- Retardo  alto  entre  disparos "},
        { Weapon::AK47, "AK47", 2000 ,"- Dispara  ráfagas  de  3 balas ,- Mantener  click:  Rafagas  cada  0.4 balas/seg,- Daño  menor  que pistola "},
        { Weapon::M3, "M3", 200 ,"- Daño  en  area  en  forma  de  cono  ,- Menor  alcance ,- Disparo  unico  por  click ,- Daño  muy alto  a  corta  distancia "}
    };
...
void Shopping::activate_shop(){
    activa = !activa;
}
bool Shopping::get_activa() const {return activa;}

void Shopping::load_items(const Weapon& clave, const std::string& name, const int& price, const SDL_Rect& destRect, const std::string& descripcion) {
    ShopItem new_item;
...
void Shopping::update(const std::vector<WeaponData>& data) {
    ...
    for (WeaponData weapon_data : data) {
        ...
        load_items(weapon_data.clave, weapon_data.name, weapon_data.price, destRect,weapon_data.descripcion);
        ++i;
    }
}
void Shopping::calculate_selection(const int& mouse_x, const int& mouse_y){

    for (const auto& item : items) {
        if((mouse_x >= item.second.destRect.x && mouse_x <= item.second.destRect.x + item.second.destRect.w) && 
            (mouse_y >= item.second.destRect.y && mouse_y <= item.second.destRect.y + item.second.destRect.h)){
                std::cout << "click EN  " << item.second.name << std::endl; 
                weapon_selec = item.first;
        }     
    }
