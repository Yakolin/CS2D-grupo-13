...
Move::Move(player_id_t player_id, MoveType move_type)
    : PlayerAction(player_id), move_type(move_type) {}
Move::~Move() {}
...
BuyWeapon::BuyWeapon(player_id_t player_id, WeaponCode weapon_code)
    : PlayerAction(player_id), weapon_code(weapon_code) {}
BuyWeapon::~BuyWeapon() {}
void BuyWeapon::action(InterfaceGame &game) {
  game.buy_weapon(this->player_id, this->weapon_code);
}

BuyAmmo::BuyAmmo(player_id_t player_id, WeaponType weapon_type,
                 ammo_t ammo_count)
    : PlayerAction(player_id), weapon_type(weapon_type),
