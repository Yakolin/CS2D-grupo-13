...
void Bomb::draw(SDL_Renderer& renderer){
    if(activada){
        origin_rect = {(item_sprite.x * width_img), (item_sprite.y * height_img), (width_img / 2), (height_img / 3)};
        if(item_sprite.y < 5){
            item_sprite.y++;
        }else{
            item_sprite.y =0;
    ...
    SDL_RenderCopyEx(&renderer, tiles_player,origin_rect, &destination_rect, angle, nullptr,SDL_FLIP_NONE);
