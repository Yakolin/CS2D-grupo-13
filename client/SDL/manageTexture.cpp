...
bool ManageTexture::load_weapons(const Weapon& id, const std::string& filePath,
    ...
    SDL_FreeSurface(surface);
    if (!texture) {
        std::cerr << "Error creando textura de weaponCWeaponCode: " << SDL_GetError() << std::endl;
        return false;
    }
...
SDL_Texture* ManageTexture::get_weapon(const Weapon& id) const {
    ...
    std::cout << "No se encontró la textura para WeaponCode: " << static_cast<int>(id) << std::endl;
