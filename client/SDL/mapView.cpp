...
TextureInfo MapView::load_zone_texture(const RectangleInfo& rectangle,const Object& zone, const SDL_Color& color){
    ...
    SDL_Rect rect= {
        rect.x = std::min(rectangle.pos_min.x, rectangle.pos_max.x) * tile_width,
        rect.y = std::min(rectangle.pos_min.y, rectangle.pos_max.y) * tile_height,
        rect.w = width * tile_width,
        rect.h = height* tile_height
    };
...
std::unordered_map<Object, TextureInfo> MapView::load_zones(const MapInfo& info ){
    ...
    map[Object::ZONE_BOMBA] = load_zone_texture(info.bomb_A,Object::ZONE_BOMBA,config.get_color_translucent(ColorTranslucent::ROJO));
    map[Object::ZONE_BOMBB] = load_zone_texture(info.bomb_B,Object::ZONE_BOMBB,config.get_color_translucent(ColorTranslucent::ROJO));
    map[Object::ZONE_COUNTERTERROSIT] = load_zone_texture(info.spawn_CT ,Object::ZONE_COUNTERTERROSIT,config.get_color_translucent(ColorTranslucent::AZUL));
    map[Object::ZONE_TERRORIST] = load_zone_texture(info.spawn_TT,Object::ZONE_TERRORIST,config.get_color_translucent(ColorTranslucent::VERDE));
    return map;
}
