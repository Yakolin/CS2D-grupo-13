...
GameLoop::GameLoop(const std::string& game_name):
        ...
        game(game_name),
        send_queues(),
        recv_queue(std::make_shared<Queue<std::unique_ptr<IActionToPlayer>>>(QUEUE_MAX_SIZE)),
...
void GameLoop::step() {
    try {
        std::unique_ptr<IActionToPlayer> action = recv_queue->pop();
