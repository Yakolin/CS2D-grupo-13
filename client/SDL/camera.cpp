...
Camera::Camera(const int& ancho, const int& alto):
        camera{0, 0, ancho, alto}, anchoVentana(ancho), altoVentana(alto) {}


void Camera::update(const float& row, const float& col, const int& playerWidth,
                    const int& playerHeight, const int& mapWidth, const int& mapHeight) {
...
int Camera::getX() { return this->camera.x; }
int Camera::getY() { return this->camera.y; }
